...
class MyDBSCAN():
    ...
    def fit(self, input_data):
        ...
        self.data=[{'val':val, 'label':0} for val in copy.deepcopy(list(input_data))]
        self.data_matrix=copy.deepcopy(input_data)
        current_label=1
    ...
    def regionQuery(self, chosen_pt, current_label):
        distance=np.sum(np.sqrt((chosen_pt['val']-self.data_matrix)**2), axis=1)
        neighbor_loc=list(np.where(distance<=self.eps)[0])
        return neighbor_loc
