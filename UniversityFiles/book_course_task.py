...
def click_every_book():
    ...
    # 2024-05-29 新增下一个控件，为了点击爱心
    FingerRoot_Child = FingerRoot.GetFirstChildControl()
    Fingercontrol = FingerRoot_Child.GetNextSiblingControl()
    location = Fingercontrol.BoundingRectangle
    # rect.left, rect.top, rect.right, rect.bottom)
    pos_x = int(location.left + (location.right - location.left)/2)
    pos_y = int(location.top + (location.bottom-location.top)/2)
    time.sleep(0.6)
    auto.Click(pos_x,pos_y)
    print('one time')
    time.sleep(2.0)
    auto.Click(pos_x,pos_y)
    print('two times')
    ...
    # (1451,338,1475,361)  rect.left, rect.top, rect.right, rect.bottom)
...
def bookTask():
    ...
    # 方法2 通过根元素获取
    DocumentCon = auto.DocumentControl(mainW,RegexName='异步社区-致力于优质IT知识的出版和分享')
